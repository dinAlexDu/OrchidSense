...
from users_database import conectar, obter_dados_utilizadores, obter_utilizador_por_email, obter_utilizador_por_id  # users_database
import sensor_database as sensor_db
from extensions import db, mail

# Configurações de upload
BASE_DIR = os.path.abspath(os.path.dirname(__file__))
UPLOAD_FOLDER = os.path.join(BASE_DIR, 'static/assets/img/illustrations/profiles')
...
def load_user(user_id):
    ...
    if utilizador:
        ...
        return User(id=utilizador_id, email=utilizador_email, password_hash=utilizador_password_hash, is_admin=is_admin, profile_image=profile_image)
    return None
...
#NOVO
# Função para obter um utilizador por email
...
#         if utilizador:
#             utilizador_id, utilizador_email, utilizador_password_hash, first_name, last_name, profile_image, is_admin = utilizador
#             # Verifica a PW com check_password_hash
#             if check_password_hash(utilizador_password_hash, password):
#                 flash('Login bem-sucedido!', 'success')
#                 return redirect(url_for('dashboard'))
#             else:
...
def sensor_lux_data_api():
    ...
    try:
        ...
        return jsonify({
                           'message': 'requisicao POST /api/sensor_lux_data sucesso e dados do sensor bh1750 recebidos corretamente'}), 200
    except Exception as e:
...
def editar_utilizador(user_id):
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            app.logger.debug(f"Tentando salvar arquivo em: {file_path}")

            # Verifique se o diretório existe e tem permissões de gravação
            if not os.path.exists(app.config['UPLOAD_FOLDER']):
                app.logger.debug(f"Diretório não encontrado: {app.config['UPLOAD_FOLDER']}")
                os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)
                app.logger.debug(f"Diretório criado: {app.config['UPLOAD_FOLDER']}")
            else:
                app.logger.debug(f"Diretório já existe: {app.config['UPLOAD_FOLDER']}")

            # Verifique permissões
            if os.access(app.config['UPLOAD_FOLDER'], os.W_OK):
                app.logger.debug(f"Permissões de gravação confirmadas para: {app.config['UPLOAD_FOLDER']}")
            else:
                app.logger.debug(f"Sem permissões de gravação para: {app.config['UPLOAD_FOLDER']}")
                raise PermissionError(f"Sem permissões de gravação para: {app.config['UPLOAD_FOLDER']}")

            file.save(file_path)
            user.profile_image = filename  # Esta linha é necessária para salvar a imagem no banco de dados
...
#
#             #  logs
#             app.logger.debug(f"A tentar gravar arquivo em: {file_path}")
#
#         db.session.commit()
#         return redirect(url_for('gestao_utilizadores'))
#
