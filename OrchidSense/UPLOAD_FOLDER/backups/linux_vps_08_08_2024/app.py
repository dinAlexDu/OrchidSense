...
def sensor_data_api():
    ...
    try:
        ...
        humidade_atual = data.get('humidade')
        data_hora = data.get('data_hora')

        if None in (temperatura_atual, humidade_atual, data_hora):
            return jsonify({'error': 'Dados incompletos, esperado temperatura, humidade e data_hora'}), 400

        # Insere os dados na base de dados
        db.inserir_dados_sensor_hora(data_hora, temperatura_atual, humidade_atual)
        ...
        return jsonify({
            ...
            'temperatura_recebida': temperatura_atual,
            'humidade_recebida': humidade_atual,
            'data_recebida': data_hora
        }), 200
...
if __name__ == '__main__':
    port = int(os.getenv('FLASK_PORT', 5000))  # Porta padrão é 5000 para consistência com o `flask run`
    debug_mode = os.getenv('FLASK_DEBUG', 'False').lower() in ['true', '1', 't']  # Modo debug baseado em variável de ambiente
