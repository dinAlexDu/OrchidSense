    ...
    with app.app_context():
        ...
        for config in user_configs:
            ...
            if (latest_data['temperatura'] < config.temp_min or latest_data['temperatura'] > config.temp_max) or \
               (latest_data['humidade'] < config.humidade_min or latest_data['humidade'] > config.humidade_max) or \
               (latest_data['luminosidade'] < config.lux_min or latest_data['luminosidade'] > config.lux_max):
...
def send_alert_email(app, email, config, data):
    with app.app_context():
        try:
            ...
            msg.body = f"""
            Condições ambientais fora dos limites para {orquidea_nome}:
            
            Temperatura actual: {data['temperatura']} (Intervalo configurado (min.-máx.): {config.temp_min}-{config.temp_max})
            Humidade actual: {data['humidade']} (Intervalo configurado (min.-máx.): {config.humidade_min}-{config.humidade_max})
            Luminosidade actual: {data['luminosidade']} (Intervalo configurado (min.-máx.):  {config.lux_min}-{config.lux_max})
            """
            mail.send(msg)
