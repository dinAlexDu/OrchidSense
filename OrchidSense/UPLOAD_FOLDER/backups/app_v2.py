...
from datetime import datetime  # Importe a classe datetime
...
def dashboard():
    ...
    # temperatura_real = 25.5  # Substitua pelo valor real da temperatura do sensor
    # humidade_real = 60.0  # Substitua pelo valor real da umidade do sensor

    # Renderizar o template com os dados das orquídeas e dos sensores
    return render_template('dashboard.html', temperatura=temperatura_atual, humidade=humidade_atual)
...
@app.route('/labenv')  # Rota para a página labenv.html
def labenv():
    return render_template('labenv.html')
...
def inserir_dados():
    data = request.get_json()
    sensor_db.inserir_dados_sensor(data['data_hora'], data['temperatura'], data['humidade'])
    return jsonify({'mensagem': 'Dados inseridos com sucesso'}), 201


@app.route('/api/dados-sensor')
def get_sensor_data():
    registros_brutos = sensor_db.recuperar_dados_sensor()
