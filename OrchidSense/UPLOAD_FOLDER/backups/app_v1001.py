...
# Configuração do Logger para capturar tudo a partir do nível DEBUG
...
def some_function():
    app.logger.debug("Esta mensagem deveria aparecer no log.")
...
def test_email():
    try:
        ...
        enviar_email('dinis.duarte@my.istec.pt', 'Teste de E-mail', 'Este é um e-mail de teste enviado pelo Flask.')
...
def index():
    app.logger.debug('Página inicial acessada')
    # Chama a função de teste ao acessar a página inicial para desenvolvimento e teste
    check_and_send_alerts()
    if not current_user.is_authenticated:
        return redirect(url_for('login'))
    return redirect(url_for('dashboard'))
